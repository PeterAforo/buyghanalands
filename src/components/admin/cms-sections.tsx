...
import {
  Loader2, HelpCircle, FolderTree, BarChart3, Settings, FileText,
  Plus, Trash2, Save, X, Star, ListOrdered, Home, Compass, Map, Shield,
  Image as ImageIcon, Mail, PanelBottom, Newspaper,
} from "lucide-react";
...
export const CMS_SECTIONS: CmsSection[] = [
  { slug: "news", label: "News Articles", icon: Newspaper },
  { slug: "hero", label: "Hero Content", icon: ImageIcon },
  { slug: "page-content", label: "Page Content", icon: FileText },
  { slug: "footer", label: "Footer Content", icon: PanelBottom },
  { slug: "testimonials", label: "Testimonials", icon: Star },
  { slug: "stats", label: "Homepage Stats", icon: BarChart3 },
  { slug: "steps", label: "How It Works Steps", icon: ListOrdered },
  { slug: "land-types", label: "Land Types", icon: Home },
  { slug: "professionals", label: "Professionals", icon: Compass },
  { slug: "regions", label: "Regions", icon: Map },
  { slug: "trust-bar", label: "Trust Bar", icon: Shield },
  { slug: "faqs", label: "FAQs", icon: HelpCircle },
  { slug: "support-categories", label: "Support Categories", icon: FolderTree },
  { slug: "contact-messages", label: "Contact Messages", icon: Mail },
  { slug: "site-settings", label: "Site Settings", icon: Settings },
];
...
export function useCmsData() {
  ...
  return { data, loading, saving, editing, setEditing, save, remove };
}
...
// --- News Tab ---
export function NewsTab({ articles, editing, setEditing, save, remove, saving }: any) {
  return (
...
// --- FAQs Tab ---
export function FaqsTab({ items, editing, setEditing, save, remove, saving }: any) {
  return (
    <div className="space-y-4">
...
export function SupportCategoriesTab({ categories, editing, setEditing, save, remove, saving }: any) {
  return (
    <div className="space-y-4">
...
// --- Homepage Stats Tab ---
export function HomepageStatsTab({ stats, editing, setEditing, save, remove, saving }: any) {
  return (
    <div className="space-y-4">
...
export function SimpleListTab({ items, entityType, editing, setEditing, save, remove, saving, fields, titleField, subtitleField }: any) {
  return (
    <div className="space-y-4">
...
// --- Regions Tab ---
export function RegionsTab({ regions, editing, setEditing, save, remove, saving }: any) {
  return (
    <div className="space-y-4">
...
// --- Hero Content Tab ---
export function HeroContentTab({ heroContent, editing, setEditing, save, remove, saving }: any) {
...
// --- Footer Content Tab ---
export function FooterContentTab({ footerContent, editing, setEditing, save, remove, saving }: any) {
    ...
    <div className="space-y-4">
      ...
      {editing && (
        <Card>
          ...
          <CardContent className="space-y-3">
            <select value={editing.section || "brand"} onChange={(e) => setEditing({ ...editing, section: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm">
            ...
            {editing.section === "links" && (
              <div>
                <label className="text-sm font-medium mb-1 block">Links (JSON array)</label>
                <Textarea rows={4} placeholder='[{"name": "About Us", "href": "/about"}]' value={editing.linksJson || ""} onChange={(e) => setEditing({ ...editing, linksJson: e.target.value })} className="font-mono text-sm" />
              </div>
            )}
            {(editing.section === "bottom" || editing.section === "brand") && (
              <div>
                <label className="text-sm font-medium mb-1 block">HTML Content</label>
                <WysiwygEditor value={editing.htmlContent || ""} onChange={(html) => setEditing({ ...editing, htmlContent: html })} placeholder="HTML content for this section..." minHeight={100} />
              </div>
            )}
            {(editing.section === "brand" || editing.section === "newsletter") && (
              <ImageUpload value={editing.imageUrl || null} onChange={(url) => setEditing({ ...editing, imageUrl: url })} label="Image (optional)" folder="cms/footer" />
            )}
            <div className="grid grid-cols-2 gap-3">
              <Input placeholder="Sort Order" type="number" value={editing.sortOrder || 0} onChange={(e) => setEditing({ ...editing, sortOrder: parseInt(e.target.value) || 0 })} />
              <label className="flex items-center gap-2 text-sm self-center"><input type="checkbox" checked={editing.isActive !== false} onChange={(e) => setEditing({ ...editing, isActive: e.target.checked })} /> Active</label>
...
export function ContactMessagesTab({ messages, save, remove }: any) {
  const statusColors: Record<string, string> = { NEW: "bg-blue-100 text-blue-700", READ: "bg-yellow-100 text-yellow-700", RESPONDED: "bg-green-100 text-green-700", ARCHIVED: "bg-gray-100 text-gray-700" };
  async function updateStatus(id: string, status: string) { await save("contactMessage", id, { status }); }
  return (
    <div className="space-y-4">
      <div className="text-sm text-gray-500">Contact form submissions from the public contact page.</div>
      <div className="space-y-2">
        {messages.length === 0 && (<Card><CardContent className="py-8 text-center text-gray-500">No contact messages yet.</CardContent></Card>)}
        {messages.map((m: any) => (
          <Card key={m.id}>
...
// --- Page Content Tab ---
export function PageContentTab({ settings, save, saving }: any) {
  ...
  useEffect(() => {
    if (currentSetting) {
      try {
        ...
        if (typeof parsed === "string") { setContent(parsed); setUseHtml(true); }
        else { setContent(JSON.stringify(parsed, null, 2)); setUseHtml(false); }
      } catch { setContent(currentSetting.value); setUseHtml(true); }
    } else { setContent(""); }
  }, [selectedPage, sectionKey, currentSetting]);

  async function handleSave() {
    let parsed;
    if (useHtml) { parsed = content; }
    else { try { parsed = JSON.parse(content); } catch { parsed = content; } }
      ...
      <Card>
        <CardContent className="py-4">
          ...
          <div className="flex gap-2 flex-wrap mb-4">
            {pageKeys.map((k) => (
              <Button key={k} variant={selectedPage === k ? "default" : "outline"} size="sm" onClick={() => setSelectedPage(k)} className={selectedPage === k ? "bg-[#1a3a2f]" : ""}>
          ...
          <div className="flex gap-2 items-center mb-3">
            ...
            <div className="flex gap-1 border rounded-lg p-0.5">
              <button onClick={() => setUseHtml(true)} className={`px-3 py-1 text-xs rounded ${useHtml ? "bg-[#1a3a2f] text-white" : "text-gray-600"}`}>WYSIWYG</button>
              <button onClick={() => setUseHtml(false)} className={`px-3 py-1 text-xs rounded ${!useHtml ? "bg-[#1a3a2f] text-white" : "text-gray-600"}`}>JSON</button>
            </div>
          </div>
          {useHtml ? (
            <WysiwygEditor value={content} onChange={setContent} placeholder="Write your page content here..." minHeight={300} />
          ) : (
            <Textarea rows={16} value={content} onChange={(e) => setContent(e.target.value)} placeholder={`{\n  "title": "...",\n  "subtitle": "..."\n}`} className="font-mono text-sm" />
          )}
        ...
        {pageContentSettings.filter((s: any) => s.key.startsWith(`page.${selectedPage}.`)).map((s: any) => (
          <Card key={s.id}>
            <CardContent className="flex items-center justify-between py-2">
              ...
              <Button size="sm" variant="ghost" onClick={() => setSectionKey(s.key.replace(`page.${selectedPage}.`, ""))}>Load</Button>
            </CardContent>
          </Card>
        ))}
      </div>
...
// --- Site Settings Tab ---
export function SiteSettingsTab({ settings, editing, setEditing, save, remove, saving }: any) {
      ...
      {editing && (
        <Card>
          <CardContent className="space-y-3 py-4">
            ...
            <select value={editing.type || "text"} onChange={(e) => setEditing({ ...editing, type: e.target.value })} className="px-3 py-2 border rounded-lg text-sm">
              <option value="text">Text</option><option value="json">JSON</option><option value="url">URL</option><option value="email">Email</option>
            </select>
            <div className="flex gap-2">
