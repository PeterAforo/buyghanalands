...
export async function GET(request: NextRequest) {
  try {
    ...
    if (moduleFilter) where.currentModule = moduleFilter;
    ...
    const workflows = await withDbRetry(() => prisma.propertyWorkflow.findMany({
      ...
      include: {
        ...
        _count: { select: { workflowDocuments: true, workflowNotes: true, workflowAlerts: true } },
      },
      orderBy: { createdAt: "desc" },
      take: 50,
    }));

    return NextResponse.json(serializeForJson(workflows));
  } catch (error) {
