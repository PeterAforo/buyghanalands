...
export async function GET(
  ...
  try {
    ...
    const versions = await withDbRetry(() => prisma.listingVersion.findMany({
      where: { listingId: id },
      include: {
...
export async function POST(
  ...
  try {
    ...
    if (listing.status !== "PUBLISHED") {
      ...
      const updated = await withDbRetry(() => prisma.listing.update({
        where: { id },
        data: {
          ...data,
    ...
    const newVersion = await withDbRetry(() => prisma.listingVersion.create({
      data: {
        listingId: id,
